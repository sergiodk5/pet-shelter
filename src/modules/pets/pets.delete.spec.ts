...
describe("DELETE /pets/:id", () => {
  ...
  it("removes the pet", async () => {
    const id = await givenAPet();
    await request(app).delete(`/pets/${id}`);
  ...
  it("removes the pet from the collection", async () => {
    const id = await givenAPet();
    expect(ids((await request(app).get("/pets")).body)).toContain(id);

    await request(app).delete(`/pets/${id}`);

    expect(ids((await request(app).get("/pets")).body)).not.toContain(id);
  });
  ...
  it("leaves every other pet alone", async () => {
    ...
    await request(app).delete(`/pets/${doomed}`);

    const res = await request(app).get(`/pets/${survivor}`);
  ...
  it("never reuses the id of a deleted pet", async () => {
    ...
    await request(app).delete(`/pets/${deleted}`);
